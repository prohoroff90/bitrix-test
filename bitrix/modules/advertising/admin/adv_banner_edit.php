				...
				if ($prop['TYPE'] == 'HTML')
				{
					...
					if (COption::GetOptionString("advertising", "USE_HTML_EDIT", "Y")=="Y" && CModule::IncludeModule("fileman")):
						...
						<div align="center" style="vertical-align:top;">
							...
							<textarea style="width:100%" rows="30" name="<?='TEMPLATE_EDITOR_'.$ind.'_'.$name?>"><?echo $strVal?></textarea>
						</div>
					<?endif;
					$html = ob_get_contents();
					ob_end_clean();
...
<form name="bx_adv_edit_form" method="POST" action="<?=$APPLICATION->GetCurPage()?>" enctype="multipart/form-data" onsubmit="changeUploaderInputs()">
<?=bitrix_sessid_post()?>
...
?>
	...
	<tr<? if (!$isEditMode) echo ' style="display: none;"'; ?> id='banner_type' valign="top">
		...
		<script type="text/javascript">
			...
			{
				var form = BX.findChild(document.body, {tagName: 'form'}, true, false);
				var arrInputs = BX.findChild(form, {tagName: 'input'}, false, true);
				for (var i in arrInputs)
				{
					var pattern = new RegExp ("^TEMPLATE_FILES", "ig");
					var fileInput = arrInputs[i].name.match(pattern);
					if (fileInput)
						form.removeChild(arrInputs[i]);
				}
			}
		</script>
		</td>
	...
	<tr id='eExtMode' style="display:none;">
		...
		<td>
			...
			<?echo InputType("checkbox", "EXTENDED_MODE", "Y", '', false, "", 'id="EXTENDED_MODE" onclick="window.oBXBannerTemplate.refreshAll()"');?>
		</td>
	</tr>
...
?>
changeType('<?echo $str_AD_TYPE?>', {params: <?=CUtil::PHPToJsObject($arPropsTemplate)?>, val: <?=CUtil::PHPToJsObject($arCurVal)?>});
</script>
<?
